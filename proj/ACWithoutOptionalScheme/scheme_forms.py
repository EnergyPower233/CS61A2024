...
def do_define_form(expressions, env, tail):
    ...
    if scheme_symbolp(signature):
        ...
        #这里必须是rest.first 不然你就丢了个链表进去 丢链表进去之后你的第一个参数(也就是这里你define的那个参数要绑定的值) 会被当成operator error返回int is not callable
        return signature
        # END PROBLEM 4
...
def do_lambda_form(expressions, env, tail = False):
    ...
    formals = expressions.first #rest is also a scheme list
    validate_formals(formals) #Scheme list
    # BEGIN PROBLEM 7
    #Accepted
    "*** YOUR CODE HERE ***"
